$(document).ready(() => {
    ...
    $.get('http://localhost:9999/product_owner/product_category',(res) => {
        var ele = "";
        for(let i=0;i<res.length;i++){
            ele += `<li class="nav-item active type" style="color : white ; margin-right:20px;" id="${res[i].product_category}"><div onClick="callBackFun(${res[i].product_category})">${res[i].product_category}</div></li>`;
        }
...
async function callBackFun(s){
    $.post('http://localhost:9999/product_owner/get_product_category_wise',{"product_category" : `${s.id}`},(response) => {
        var str = '';
        for(let i=0;i<response.length;i++){
